...
const HorizontalList = ({ movies, genres, theme = 'light' }) => {
  ...
  return (
    ...
    <div className="relative mx-12 pt-4">
      ...
      {/* Left Button */}
      <button className={`absolute left-0 top-1/2 transform ${movies ? "-translate-y-1/2" : "-translate-y-1/3"} rounded-full p-2`} onClick={scrollLeft} >
      ...
      {/* List */}
      <div ref={scrollRef} className={`flex space-x-4 overflow-x-auto scrollbar-none py-4`} >
        {/* Movie */}
        {
          movies ? (
            ...
            genres.map((genre, index) => (
              <GenreCard genre={genre} theme={theme} index={index} />
            ))
          )
        }
      ...
      {/* Right Button */}
      <button className={`absolute right-0 top-1/2 transform ${movies ? "-translate-y-1/2" : "-translate-y-1/3"} rounded-full p-2`} onClick={scrollRight} >
        <span className="inline-flex items-center justify-center w-10 h-10 rounded-full bg-white/30 hover:bg-white/70">
          <svg className="w-4 h-4 text-black" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 6 10">
