...
const genres = [
  ...
  { name: "Music", value: "music", icon: faTheaterMasks },
...
const DiscoverPage = () => {
  ...
  const movieIds = [
    122,4011,420634,475557,533535,567811,619264,663712,698687,726139,748230,807339,877817,889737,917496,923667,933260,945961,949484,957452,1022789,1029281,1034541,1052280,1062215, 
    1064028,1066262,1079091,1087822,1114513,1125510,1129598,1139817,1147710,1160018,1184918,1186532,1186947,1190868,1207830,1215162,1226578,1248753
  ];
  const popularIds = [278, 238, 240, 424, 389, 129, 19404, 155, 496243, 497, 680, 13, 372058, 122,769, 429, 346, 12477, 157336, 324857, 1891, 244786, 569094, 27205, 18491,105, 128, 274, 620249];
  const showIds = [112470, 82708, 247174, 247884, 72879,260196, 262252, 75269, 61818, 2224, 1416, 2734, 2051, 114439, 549, 1871, 1489, 75219, 103147, 92621];
  const popularShowIds = [1434, 1433, 131041, 456, 4614, 1399, 79744, 17404, 12971, 1408, 1405, 1421, 63174, 71712, 37680, 1396, 30984, 60574, 95479, 76479];
  ...
  useEffect(() => {
    const fetchProfile = async () => {
      ...
      else setTrendingMovies(data);
      //
      const { data: d, error: e } = await supabase
        .from("Movies")
        .select()
        .in("movie_id", popularIds);
      if (e) console.log(e);
      else setPopularMovies(d);
      //
      ...
      else setTrendingShows(da);
      //
      const { data: dat, error: err } = await supabase
        .from("Shows")
        .select()
        .in("show_id", popularShowIds);
    ...
    <div className={`ml-[100px] min-h-screen `}>
      ...
      <DiscoverCarousel
        movies={[
          {
            image:
              "https://4kwallpapers.com/images/wallpapers/oppenheimer-8k-2023-3840x1080-12220.jpg",
            movie_id: 872585,
          },
          {
            image:
          ...
          {
            image: "https://images8.alphacoders.com/129/1297243.png",
            movie_id: 315162,
          },
        ]}
      />
